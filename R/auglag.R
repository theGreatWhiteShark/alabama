                    ...
                    ...) 
{
  ...
  ## #####################################################################
  ## Setting up the default configuration and integrate provided arguments
  control.outer.default <- list(lam0 = 10, sig0 = 100, eps = 1e-07,
                                itmax = 50, method = "BFGS", trace = TRUE,
                                NMinit = FALSE, ilack.max=6,
                                i.scale = 1, e.scale = 1, kkt2.check=TRUE)

  control.optim.default <- list(trace = 0, fnscale = 1, parscale = rep.int(1,length(par)),
                                ndeps = rep.int(0.001, length(par)), maxit = 100L,
                                abstol = -Inf, reltol = sqrt(.Machine$double.eps), alpha = 1,
                                beta = 0.5, gamma = 2, REPORT = 10, type = 1, lmm = 5,
                                factr = 1e+07, pgtol = 0, tmax = 10, temp = 10)

  control.nlminb.default <- list(eval.max=200,iter.max=150,trace=0,abs.tol=0,
                                 rel.tol=1.e-10,x.tol=1.5e-08,xf.tol=2.2e-14,
                                 step.min=1,step.max=1,sing.tol=1.e-10)

  control.outer <- modifyList(control.outer.default, control.outer) 

  control.inner <- if (control.outer$method != "nlminb"){
                     modifyList(control.optim.default,control.optim)
                   } else if (control.outer$method=="nlminb"){
  ...
  ## #####################################################################
  ## Initializing wrapper functions to start the calculation. There are three
  ## wrappers: auglag1 for equality constraints only, auglag2 for inequality
  ## constraints only, and auglag3 for both
...
##################################################################
######### Wrapper function for equality constraints only #########
##################################################################
...
##################################################################
####### Wrapper function for inequality constraints only #########
##################################################################
                     ...
                     ...) 
{
  ...
  ## #############################################################
  ## Defining a version of the function to be evaluated and its
  ## gradient that are both aware of the constraints
  fun <- function(par, ...) {
    ## Evaluate the inequality constraints (returns numerical vector)
    h0 <- hin(par, ...)
    d0 <- h0
    ...
    fn(par, ...) - pfact * sum(lam * d0) + 
      pfact * sig/2 * sum(d0 * d0) # Nocedal P. 514; (17.36)
  }
  ...
  geval <- 0
  ilack <- 0
  Kprev <- dmax # just redefining everything to let the read loose track
  ## Rescaling the penalty coefficient for the quadratic term, so
  ## the maximal constraint violation is equal to the Lagrangian
  ## multiplier.
  sig0 <- sig/Kprev
  if (is.infinite(sig0)) 
    sig0 <- 1
  ...
  for (i in 1:itmax) {
    ...
    r.old <- r
    ## #############################################################
    ## The stats::optim or stats::nlminb functions will be used
    ## for optimization in the inner routine.
    if (NMinit & i == 1){
      ## Using the Nelder-Mead downhill simplex algorithm for the
      ## first iteration.
      a <- optim(par = par, fn = fun, 
                 control = control.optim, method = "Nelder-Mead", ...)
    }
      ...
      if (sig > 1e+05){
        ## Increase the tolerance up to which the optim algorithm
        ## will continue optimizing the function to compensate big
        ## penalty parameters. Since the penalty term will dominate
        ## Lagrangian the influence of the original function is getting
        ## smaller and thus the optimization routine has to become
        ## more sensitive.
        control.optim$reltol <- 1e-10
      }
      a <- optim(par = par, fn = fun, gr=gradient, control = control.optim,
                 method = method, ...)
      r <- a$value
      ## Feed the global counts of function and gradient evaluation.
      feval <- feval + a$counts[1]
      geval <- geval + a$counts[2]
    } else if (method == "nlminb") {
      a <- nlminb(start = par, objective = fun, gradient=gradient, control=control.optim, ...)
    ...
    ## #############################################################
    ## Outer routine
    ## Re-initialize the above optimization by calculating the new
    ## Lagrange multipliers (lam) and penalty parameters (sig) used
    ## within the function minimized by the inner routine.

    ## Evaluate the constraints with the newly found parameters
    par <- a$par
    h0 <- hin(par, ...)
    d0 <- h0
    inactive <- (1:length(h0))[(h0 > lam[1:length(h0)]/sig)]
    d0[inactive] <- lam[inactive] / sig
    K <- max(abs(d0))
    ## Update the Lagrange multiplier (by the amount of the individual
    ## violation multiplied by the penalty parameter) only if the
    ## biggest constraint violation is at least four times smaller
    ## than the one from the previous update. If the violation is
    ## still bigger, increase the penalty parameter instead.
    if (K <= Kprev/4) {
      lam <- lam - d0 * sig # Nocedal P. 515; (17.39)
      Kprev <- K
    }
    else sig <- 10 * sig
    obj <- fn(par, ...)

    ## Check if there was a change in the parameter estimate.
    ## Only a certain (ilack.max) number of rounds with the
    ## maximal parameter update below a certain value will be tolerated.
    pconv <- max(abs(par - par.old))
    if (pconv < eps) {
      ilack <- ilack + 1
    } else ilack <- 0 

    ## Stop the optimization as soon as the update of the Lagrangian
    ## and the biggest constraint violation is smaller than eps or
    ## ilack.max time the parameters just obtained a negligible
    ## increment.
    if ((is.finite(r) && is.finite(r.old) && abs(r - r.old) < 
         eps && K < eps) | ilack >= ilack.max) break
  }
  ...
  ## ###############################################################
  ## Wrap up. Writing an output object and checking the second-order
  ## sufficient conditions for the optimization (Whether the point
  ## is really a maximum or minimum).
  if (i == itmax) {
    a$convergence <- 7
    a$message <- "ALABaMA ran out of iterations and did not converge"
  } 
